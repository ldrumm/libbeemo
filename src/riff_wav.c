...
BMO_buffer_obj_t *bmo_fopen_wav(const char *path, uint32_t flags)
{
    ...
    /*gets information from the wav header and returns a structure with floating
     * point audio data etc*/

    /*based on original wav header info from Wed 26 Dec 2007 03:03:21 GMT by
     * me, Luke Drummond*/
    /*rewritten for AUD Fri 02 Sep 2011 02:28:58 BST */
    /*TODO:recognise the following subchunk ids and do something useful -
     probably ignore them:
        'bext' chunk found in files recorded in protools. seems to come
        directly after WAVE, and contain some sort of program metadata 'minf'
        again found in protools files as part of the 'fmt ' chunk content 16
        bytes and purpose unknown 'elm1' subchunk of 'fmt ':- contained 7kiB
        of null bytes. go protools.
    */
    BMO_wav_header header;
    void *data = NULL;
    char *hp = NULL; // wav header pointer
    uint32_t temp;
    //    uint32_t samples = 0; //warning: variable ‘samples’ set but not used
    //    [-Wunused-but-set-variable]
    size_t size = 0;
    uint16_t temp16;
    //    int tofloat;    //TODO warning: variable ‘samples’ set but not used
    //    [-Wunused-but-set-variable]
    int fact = 0;
    int be = bmo_host_be();
    int swap = 0;
    ...
    if (header.chunk_id == 0x46464952) {
        //'RIFF' in little endian decoding
        rifx = 0;
        if (be)
            swap = 1;
        bmo_debug("%s is a RIFF\n", path);
    } else if (header.chunk_id == 0x58464952) {
        //'RIFX' denotes a big endian file
        rifx = 1;
        if (!be)
            swap = 1;
    ...
    //
    //    default:
    //        bmo_info(stderr,  "%d is an unrecognised wavformat
    //        code\n",header.format_no);
    ...
    if (fact) {
        hp += 2;
        if ((*(uint32_t *)hp) == 0x74636166 ||
            (*(uint32_t *)hp) == 0x66616374) {
            hp += 4;
    ...
    /*do the actual copying of audio data and set up the audiotrack struct with
     * relevant information */
    hp += 4;
    //    samples = header.subchunk_2_size / (header.bits_per_sample / 8);
    if (header.subchunk_2_size > size) {
        bmo_err(
            "AUD ERROR: wav file malformed. requested read longer than file.");
        // FIXME cleanup before return otherwise memory leak
        return NULL;
    }
    ...
    return bmo_bo_new(
        flags, header.num_channels,
        (header.subchunk_2_size / bmo_fmt_stride(flags) / header.num_channels),
        header.sample_rate, 0, size, (void *)hp);
} /*bmo_fopen_wav*/


// static char * BMO_gen_wav_header(int samples, int rate, int channels,
// uint32_t fmt)
//{
//    int i = 0;
//    int resolution;
...
//        default:assert(0); //user should not be calling this function with any
//        other fmt
//    }
//
//    (void)floating; //FIXME conform to wave format extensible in case of
//    floating point
//
...
//    header.subchunk_1_size = 16;                    (pcm == 1) ?
//    WAVE_FORMAT_PCM : WAVE_FORMAT_IEEE_FLOAT;// this should be
//    header.format_no = (uint16_t) (pcm == 1) ? WAVE_FORMAT_PCM :
