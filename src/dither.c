...
int8_t bmo_rand_tpdf8(void)
{
	...
	the static variables allow one random call per invocation halving rand() calls
	*/
	static int8_t state;
...
void bmo_dither_tpdf(void * stream, uint32_t fmt, size_t samples)
{
	...
	/**
	dithers a PCM stream of arbitrary resolution by setting the least significant bit with the output
	of a TPDF random number generator.
	
	Here the focus is on speed.  Some Dither functions ADD the output of a urand function but involves the possibility of integer overflow
	which must be caught and reversed with a 'test-and-branch' in the inner loop.
	...
	*/
	...
	switch (stride)
	{
		...
		default:
			bmo_err("cannot dither %dbit streams\n", stride *8);
	}		
}
