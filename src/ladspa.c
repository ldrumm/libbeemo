...
static LADSPA_Data
normalise_port_val(const LADSPA_Descriptor *ld, unsigned long i, uint32_t rate, LADSPA_Data val)
{
    ...
    val = LADSPA_IS_HINT_BOUNDED_ABOVE(hd) ? MIN(hint->UpperBound, val): val;
...
static LADSPA_Data
default_port_val(const LADSPA_Descriptor *ld, unsigned long i, uint32_t rate, float A440)
{
    const LADSPA_PortRangeHint * hint = ld->PortRangeHints + i;
    const LADSPA_PortRangeHintDescriptor hd = hint->HintDescriptor;
...
static void
print_port_default(const LADSPA_Descriptor * ld, unsigned long i, uint32_t rate, float A440)
{
    const LADSPA_PortRangeHint * hint = ld->PortRangeHints + i;
    const LADSPA_PortRangeHintDescriptor hd = hint->HintDescriptor;
    printf("Port %lu:%s:%s%s%s%s::%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s>=%1.1f<=%1.1f-->%f\n", \
        i+1, ld->PortNames[i],
...
bmo_load_ladspa(const char * path, unsigned long index)
{
...
_bmo_init_dsp_ladspa(void * in, uint32_t flags)
{
	...
	for(unsigned long i = 0; i < ld->PortCount; i++){
		if(LADSPA_IS_PORT_CONTROL(ld->PortDescriptors[i])){
			...
			state->ctl_ports[ctlports] = default_port_val(ld, i, dsp->rate, CONCERT_A);
			ctlports++;
		}
	}
...
bmo_dsp_ladspa_new(const char *path, uint32_t flags, uint32_t channels, uint32_t frames, uint32_t rate)
{
    ...
    if(n_audio_inputs(ld) > dsp->channels ||
        n_audio_outputs(ld) > dsp->channels){
            bmo_err("'%s'requires %lu inputs and %lu outputs;\n"
