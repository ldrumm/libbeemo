...
void bmo_dither_sb(float *buffer, uint32_t resolution)
{
    (void)buffer;
    (void)resolution;
    BMO_NOT_IMPLEMENTED;
...
double bmo_osc_sq_mix_sb(float *buffer, float freq, double phase,
                         float amplitude, uint32_t rate, uint32_t samples)
{
    ...
    // not a true odd harmonic sine-additive squarewave. It has a low slew rate,
    // and only an approximation of a true square wave but could be accepted
    // considering its execution speed(it's faster than audacity)
    float y0, y1, y2, w;
    w = freq * 2.0 * M_PI / rate;
    float b1 = 2.0 * cos(w);
    float x;
    y1 = sin(phase - w);
    y2 = sin(phase - 2 * w);
    // all samples are halved during clamping, so to get the requested
    // amplitude, we must double again.
    amplitude *= 2;
...
double bmo_osc_sq_mix_sbB(
    float *buffer, float freq, double phase,
    float amplitude, uint32_t rate, uint32_t samples) // TODO phase offset, frequency
{
    (void)phase;
    float w;
    w = freq * (2.0 * M_PI / rate);
    while (samples--) {
        buffer[samples] +=
            (amplitude *
             sin(2. * cos(-2. * cosh(2. * cos(w * samples * 0.125)))));
    }
    return phase;
}

double bmo_osc_saw_sb(
    ...
    uint32_t rate, uint32_t samples)
{
    ...
    while (samples--)
        buffer[samples] +=
            ((inc * ((samples + (int)phase) % (int)a1)) - amplitude);
    return phase;
}
...
void bmo_inv_sb(float *buffer, uint32_t samples)
{
    ...
    if (samples % 4 == 0) {
        samples /= 4;
        while (samples--)
            vec_buf[samples] ^= sign_vec;
    } else {
        while (samples--)
            int_buf[samples] ^= (sign);
    }
}
...
//      return;
//  }
    bmo_zero_sb(out, samples);
    /*convolveHack input with impulse*/
    // for (uint32_t i = 0; i < samples; i++, tmp++) {
    for (uint32_t i = 0; i < samples; i++) {
        float f = in[i];
        for (uint32_t j = 0; j < impulselen; j++) {
            if (j + i < samples)
                // tmp[j] += impulse[j] * f; // TODO vectorize this mixing code
                out[j] += impulse[j] * f; // TODO vectorize this mixing code
        }
    }
...
#define MB_SB_WRAPPER(name, function)                                          \
    void bmo_##name(float **buf, uint32_t channels, uint32_t frames)           \
    {                                                                          \
        for (uint32_t i = 0; i < (channels); i++)                              \
            bmo_##function(buf[i], (frames));                                  \
    }

#define MB_SB_OSC_WRAPPER(name, function)                                      \
    double bmo_##name_mb(float **buffer, float freq, double phase,             \
                         float amplitude, uint32_t rate, uint32, channels,     \
                         uint32_t frames)                                      \
    {                                                                          \
        double ret;                                                            \
        for (uint32_t i = 0; i < channels, i++)                                \
            ret = bmo_##function(buffer[i], freq, phase, amplitude, rate,      \
                                 frames);                                      \
        return ret;                                                            \
    }
