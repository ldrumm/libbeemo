...
uint32_t bmo_wait_ipc_driver(BMO_state_t *state)
{
    /** Blocks until the realtime callback completes and signals via IPC.
        This can be implemented as a write/read pipe or blocking on a semaphore
        etc, and this just wraps that platform specific functionality.
