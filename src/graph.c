...
int bmo_update_dsp_tree(BMO_dsp_obj_t *node, uint64_t tick, uint32_t flags)
{
    ...
    are satisfied.  This function does not attempt to detect cycles, as it
    assumes that the graph is acyclic. However, as each dependency is atomically
    updated with the current tick value on function entry and this, acts as a
    conditional for the update we shouldn't see infinite loops, just silence
    returned from subgraphs that have dependency as the update will not visit
    any leaf nodes on the given subtree.
    */
    ...
    if (dependency) {
        // If we will be mixing data into the current input buffers, we want
        // them to
        // be zero beforehand.  This only occurs when this node is a dependant
        // of other nodes.
    ...
    while (dependency) {
        ...
        uint32_t channels =
            MIN(node->channels, ((BMO_dsp_obj_t *)dependency->data)->channels);
        bmo_mix_mb(node->in_buffers, node->in_buffers,
                   ((BMO_dsp_obj_t *)dependency->data)->out_buffers, channels,
                   node->frames);
        dependency = dependency->next;
    }
    dependency = node->ctl_ports;
    while (dependency) {
        BMO_NOT_IMPLEMENTED; // FIXME control port directional semantics haven't
                             // been decided yet.
        bmo_update_dsp_tree(dependency->data, tick, flags);
        dependency = dependency->next;
    }
...
)
{
    ...
    */
    ...
    BMO_ll_t *dependency = node->in_ports;
    int ret = callback(node, userdata);
    if (ret) {
        return ret;
    };
    while (dependency) {
        ret = bmo_traverse_dag(dependency->data, callback, userdata);
        if (ret) {
            return ret;
        }
        dependency = dependency->next;
