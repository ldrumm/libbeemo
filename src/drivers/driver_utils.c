...
{
	return state->n_playback_ch;///counter intuitive?  these are inputs /into/ the driver similar to inputs /into/ a hifi amp. This terminology comes from JACK
}
...
const char * bmo_strdriver(BMO_state_t * state)
{
	switch(state->driver_id)
	{
		case BMO_DUMMY_DRIVER:
			return "AUD dummy driver writing to disk";
		case BMO_JACK_DRIVER:
			return "AUD Jack driver";
		case BMO_PORTAUDIO_DRIVER:
			return "AUD Portaudio Driver";
		default:return NULL;
	}
}
...
uint32_t bmo_driver_ver(BMO_state_t * state)
{
	...
	{
		case BMO_DUMMY_DRIVER:
			return BMO_VERSION_MAJOR;
		case BMO_JACK_DRIVER:
			return 0;	//jack does not seem to have a way to query the server version 
		case BMO_PORTAUDIO_DRIVER:
		#ifdef BMO_HAVE_PORTAUDIO
...
uint32_t 
bmo_bufsize(BMO_state_t * state)
{
	return state->buffer_size;
}

void 
bmo_start(BMO_state_t * state)
{
...
uint32_t 
bmo_status(BMO_state_t * state)
{
    assert(((state->dsp_flags & (BMO_DSP_RUNNING | BMO_DSP_STOPPED)) ^ (BMO_DSP_RUNNING | BMO_DSP_STOPPED)) != 0);
