...
BMO_ringbuffer_t *bmo_init_rb(uint32_t frames, uint32_t channels)
{
    ...
    bmo_zero_mb(rb->data, channels, tot_len);
...
uint32_t bmo_write_rb(BMO_ringbuffer_t *rb, float **in_buf, uint32_t frames)
{
    ...
    while (to_write) {
        write =
            (rb->write_index + to_write < rb->frames)
                    ? to_write
                    : rb->frames - rb->write_index;
        ...
        if (rb->write_index >= rb->frames) {
            // once at the end return to start.
            rb->write_index = 0;
        }
    }
...
uint32_t bmo_read_rb(BMO_ringbuffer_t *rb, float **out_buf, uint32_t frames)
{
    ...
    while (frames_left) {
        to_read = (rb->read_index + frames_left < rb->frames)
                      ? frames_left
                      : rb->frames - rb->read_index;
        for (i = 0; i < rb->channels; i++) {
