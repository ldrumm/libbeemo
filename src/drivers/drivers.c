...
BMO_state_t * bmo_driver_init(
    BMO_state_t *state,
    uint32_t channels, uint32_t rate, uint32_t buf_len, uint32_t flags,
    void *data
)
{
    if (!channels || !rate || !buf_len) {
        ...
        if (!channels)
            channels = BMO_DEFAULT_CHANNELS;
        if (!rate)
            rate = BMO_DEFAULT_RATE;
        if (!buf_len)
            buf_len = BMO_DEFAULT_BUF;
    }
    ...
    switch (flags & (BMO_PORTAUDIO_DRIVER | BMO_JACK_DRIVER | BMO_DUMMY_DRIVER)) {
        ...
        #ifdef BMO_HAVE_PORTAUDIO
            bmo_debug(
                "starting Portaudio driver with rate=%d, channels=%d, buf_len=%d\n",
                rate,
                channels,
                buf_len
            );
        ...
        case BMO_JACK_DRIVER: {
        #ifdef BMO_HAVE_JACK
            bmo_debug(
                "starting Jack driver with client name '%s'"
            ...
            return bmo_jack_start(
                state, channels, rate, buf_len, flags, (const char *)data
            );
        #else
        ...
        case BMO_DUMMY_DRIVER: {
            bmo_debug(
                "starting dummy driver with file name '%s'"
                "rate=%d, channels=%d, buf_len=%d\n",
                (const char *)data,
                rate,
                channels,
