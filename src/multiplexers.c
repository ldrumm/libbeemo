...
pcmtoint64(char *in, int swap_in, uint32_t stride_in, int64_t range)
{
    int64_t tmp = 0x0000000000000000;
#ifdef BMO_ENDIAN_LITTLE
    const uint32_t offset = 0;
#else
    const uint32_t offset = 8 - stride_in;
#endif
    char *cp = (char *)&tmp;
    if (swap_in) {
        for (uint32_t i = 0, k = stride_in - 1; i < stride_in; i++, k--)
            cp[i + offset] = in[k];
    } else {
        for (uint32_t i = 0; i < stride_in; i++)
            cp[i + offset] = in[i];
    }
    // sign carry and convert to signed if input is unsigned.
    if (tmp >= (range / 2)) {
        tmp = 0 - (range - tmp);
    }
    return tmp;
...
static inline void
int64topcm(char *out, int64_t in, int swap_out, uint32_t stride)
{
    uint32_t offset;
    int64_t tmp = in;
#ifdef BMO_ENDIAN_LITTLE
...
void _bmo_swap_ib(void *data, uint32_t stride, size_t len)
{
    ...
    switch (stride) {
        case 1:
            return; // single byte variables don't have endianness - do nothing
        case 2: {
            ;
            uint16_t *p16 = data;
            while (i < len) {
...
{
    return flags & (BMO_DITHER_SHAPED | BMO_DITHER_TPDF);
}
...
void bmo_conv_iftoif(
    void *out, void *in,
    uint32_t fmt_out, uint32_t fmt_in,
    uint32_t samples
)
{
	bmo_debug("\n");
    ...
    switch (bmo_fmt_stride(fmt_in)) {
        case 4: {
            ...
            } else if (bmo_fmt_stride(fmt_out) == sizeof(double)) {
                // float to double
                fp = (float *)in;
                dp = (double *)out;
        ...
        case 8: {
            ...
            } else if (bmo_fmt_stride(fmt_out) == sizeof(float)) {
                // double to float
                dp = (double *)in;
                fp = (float *)out;
                for (uint32_t i = 0; i < samples; i++) {
...
)
{
    ...
    int swap_in = must_swap(fmt_in);
    int swap_out = must_swap(fmt_out);

    uint32_t i;
    char *out = (char *)out_stream;

    int64_t pcm;
    if (bmo_fmt_stride(fmt_in) == 4) {
        // standard float
        float f;
        ...
        for (i = 0; i < count; i++) {
            ...
            int64topcm(out, pcm, swap_out, stride_out);
            out += stride_out;
        }
    } else if (bmo_fmt_stride(fmt_in) == 8) {
        // standard double
        double d;
...
)
{
    ...
    /** copies the interleaved data from in_stream to out_stream, converting the
    sample data as requested.
    TODO where possible (i.e. where conversion is int -> int), integer only
    arithmetic is used to speed up the computation*/
    ...
    At present, normalise all input samples to signed 64bit, then normalize to
    output format to avoid conditional in inner loop
    */
    int swap_in = must_swap(fmt_in);
    int swap_out = must_swap(fmt_out);
    ...
    int64_t factor =
        (int64_t)(_bmo_fmt_pcm_range(fmt_in) / _bmo_fmt_pcm_range(fmt_out));
    uint32_t shrink = ((stride_in > stride_out) ? 1 : 0);
    int64_t range = (int64_t)_bmo_fmt_pcm_range(fmt_in);
    int64_t pcm;

    for (uint32_t f = 0; f < count; f++) {
        pcm = pcmtoint64(in, swap_in, stride_in, range);
        in += stride_in;
...
)
{
    ...
    for (uint32_t f = 0; f < frames; f++) {
        for (uint32_t ch = 0; ch < channels; ch++) {
            output_data[ch][f] =
                (float)pcmtoint64(in, swap_in, stride_in, range) * multiplier;
            in += stride_in;
        }
    }
}
...
)
{
    bmo_debug("\n");
    assert((!bmo_fmt_pcm(fmt_in)));
    // FIXME can't yet read from double precision buffer
    assert(bmo_fmt_stride(fmt_in) == sizeof(float));
    uint32_t count = channels * frames;
    uint32_t swap = must_swap(fmt_in);
...
{
    bmo_debug("\n");
    // TODO add double precision float output
    assert((bmo_fmt_pcm(fmt_out) | (bmo_fmt_stride(fmt_out) == 4)));
    ...
    switch (bmo_fmt_pcm(fmt_out)) {
        case 1: {
            ...
            for (uint32_t f = 0; f < frames; f++) {
                for (uint32_t ch = 0; ch < channels; ch++) {
                    temp = (int64_t)lrint(
                        ((double)in[ch][f] * (double)multiplier));
                    int64topcm(out_buf, temp, swap_out, stride_out);
                    out_buf += stride_out;
                }
            }
            break;
...
)
{
    ...
    switch (_bmo_fmt_conv_type(fmt_out, fmt_in)) {
        ...
        case BMO_INT_TO_FLOAT: {
            // FIXME nteger PCM to double precision float conversion
            assert(bmo_fmt_stride(fmt_out) == sizeof(float));
            bmo_conv_ipcmtoif(out, in, fmt_out, fmt_in, count);
            break;
        }
        case BMO_FLOAT_TO_FLOAT: {
            // TODO
        ...
        case BMO_FLOAT_TO_INT: {
            ...
            bmo_conv_iftoipcm(out, in, fmt_out, fmt_in, count);
            break;
        }
    }
}
