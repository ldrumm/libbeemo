...
#define AU_BMO_SUPPORTED_FORMATS \
    (BMO_FMT_PCM_8 | BMO_FMT_PCM_16_BE | BMO_FMT_PCM_24_BE | BMO_FMT_PCM_32_BE \
    | BMO_FMT_FLOAT_32_BE|BMO_FMT_FLOAT_64BE)
...
{
	BMO_au_header_t header = {0, 0, 0, 0, 0, 0, 0, NULL};
	...
	else if(header.au_magic_number == 0x646e732e){
	    //"dns." indicates a little-endian encoded file
        file_le = 1;
    }
    else{
        bmo_err("%s is not a valid AU file:magic number is 0x%8x\n",
            path, header.au_magic_number);
        goto fail;
    }
    ...
    if(header.au_data_offset >= AU_HEADER_SIZE ){
        ...
        //the spec requires the metadata to end with an ASCII NUL, but who knows...
    ...
    if(!header.au_data_size){
        //It is valid to have a zero-length audio stream, but is there any point?
    ...
    switch(header.au_data_encoding){
        ...
        case AU_FORMAT_16_BIT_PCM: flags |= ((file_le == 0) ? BMO_FMT_PCM_16_BE   : BMO_FMT_PCM_16_LE);   break;
        case AU_FORMAT_24_BIT_PCM: flags |= ((file_le == 0) ? BMO_FMT_PCM_24_BE   : BMO_FMT_PCM_24_LE);   break;
        case AU_FORMAT_32_BIT_PCM: flags |= ((file_le == 0) ? BMO_FMT_PCM_32_BE   : BMO_FMT_PCM_32_LE);   break;
        case AU_FORMAT_32_BIT_FLOAT:flags |= ((file_le == 0) ? BMO_FMT_FLOAT_32_BE : BMO_FMT_FLOAT_32_LE); break;
        case AU_FORMAT_64_BIT_FLOAT:flags |= ((file_le == 0) ? BMO_FMT_FLOAT_64BE  : BMO_FMT_FLOAT_64_LE); break;
        default:{
...
int bmo_fwrite_header_sun(FILE *file, uint32_t flags, uint32_t channels, uint32_t frames, uint32_t rate)
{
    ...
    /*
    Sun's specification states that metadata must be a non-zero multiple of 8 bytes
...
@returns size of the resultant file 0 on failure.  If the write fails, the state
of @file are unspecified.
...
size_t bmo_fwrite_sun(BMO_buffer_obj_t *buffer, FILE *file, uint32_t flags)
{
	...
	if(bmo_fwrite_header_sun(file, flags, buffer->channels, buffer->frames, buffer->rate) != 0){
