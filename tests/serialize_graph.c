...
static char output_file[BUF_LEN] = {'\0'};
...
static size_t max_consecutive_x(const char *restrict s, char x)
{
    /** find the maximum number of repetitions of the given char `x` in string s */
    #ifndef MAX
...
static char *strchrsubs(char *restrict s, char old, char new)
{
    while (*s) {
        if (*s == old)
            *s = new;
...
static char *serialize_lua_object(BMO_dsp_obj_t *node)
{
    const BMO_lua_context_t *ctx = node->userdata;
    const char *const user_script = ctx->user_script;
    const size_t user_script_len = ctx->user_script_len;
    const size_t n_equals_signs = max_consecutive_x(user_script, '=') + 1;
    char *restrict open_brackets = calloc(n_equals_signs + 3, sizeof(char));
    char *restrict close_brackets = calloc(n_equals_signs + 3, sizeof(char));
    ...
    const size_t serialized_len = (n_equals_signs * 2) + 4 + user_script_len + 1;
    char *restrict serialized = calloc(serialized_len, sizeof(char));
    snprintf(
        serialized,
        serialized_len,
        "%s%s%s",
...
static char *serialize_ladspa_object(BMO_dsp_obj_t *node)
{
    BMO_NOT_IMPLEMENTED;
    (void) node;
    return NULL;
...
static char *serialize_bo_object(BMO_dsp_obj_t *node)
{
    BMO_NOT_IMPLEMENTED;
    (void)node;
    return NULL;
...
static char *serialize_rb_object(BMO_dsp_obj_t *node)
{
    BMO_NOT_IMPLEMENTED;
    (void)node;
    return NULL;
...
static char *serialize_deps(BMO_dsp_obj_t *node)
{
    // first, we count how many characters we're going to need in the malloced
    // buffer.
    ...
    static const char *const fmt_string = "%" PRIu64;
    char buf[22] = {0}; //(2^64-1) as a decimal char string is 20 bytes
    size_t n_chars = 0;
    BMO_ll_t *dep = node->in_ports;
    while (dep) {
        uint64_t id = ((BMO_dsp_obj_t *)dep->data)->id;
        snprintf(buf, sizeof(buf) - 1, fmt_string, id);
        n_chars += strlen(buf);
        dep = dep->next;
    }
    ...
    // Do the actual filling of the buffer now we know how long the result
    // string will be
    dep = node->in_ports;
    while (dep) {
        uint64_t id = ((BMO_dsp_obj_t *)dep->data)->id;
...
int serialize_callback(BMO_dsp_obj_t *node, void *userdata)
{
    ...
    const struct bmo_dsp_serializer_callback_reg_t *serializer =
        get_dsp_serializer(serializers, node);
    ...
    const char *data = serializer->serialize(node);
    fprintf(
        serializers->file,
        "dsp{id=%" PRIu64 ", type=\"%s\", data=%s, "
        "inputs={%s}, channels=%d, rate=%d}\n",
        node->id,
        serializer->name,
        data,
        deps,
...
)
{
    ...
    bmo_traverse_dag(node, serialize_callback, &userdata);
    fflush(file);
    return fclose(file);
}
...
int main(void)
{
    /**
        ...
        After calling bmo_update_dsp_tree() on the bottom of the graph, we
expect to
...
dsp_top_a       dsp_top_b
    ...
    */
    ...
    for (size_t i = 0; i < N_DSPS; i++) {
        dsps[i] = bmo_dsp_lua_new(
            0,
            CHANNELS,
            FRAMES,
            RATE,
            "a = [=[My name is ====]]]=];function dspmain() os.time() end",
            0
        );
    ...
    char luac_command[BUF_LEN] = {'\0'};
    snprintf(luac_command, BUF_LEN - 1, "luac -p \"%s\"", output_file);
    printf("%s\n", luac_command);
