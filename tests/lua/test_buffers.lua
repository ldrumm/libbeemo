function garbagebuf(channels, frames)
...
buffer_tests = TestingUnit{
    ...
    assert_buf_data_equal = function(self, x, y)
        if x.channels ~= y.channels or x.frames ~= y.frames then return self:_assertion_failure{
    ...
    test_buffer_table_call = function(self)
        --[[ buffers can be be called with a function or table argument.
            The contents of a table argument should be copied into the callee's internal buffer
            A function argument must be called with the buffer table as it's only argument
        ]]
        local buf = dsp.sys()
        buf:zero()
        ...
        buf{write_garbage}  --buf should now call this function and be filled with garbage
